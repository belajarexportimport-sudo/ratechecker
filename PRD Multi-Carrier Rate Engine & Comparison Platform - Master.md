...
**Status:** Master PRD — Phase 1 (FedEx Foundation) hampir selesai, lihat §49
untuk status implementasi terkini  
**Tanggal:** 7 September 2026 (dibuat) | **Diupdate:** 8 September 2026  
...
Phase 1 dianggap selesai jika (status per 8 Sep 2026, detail lengkap di §49):

- [x] `core/schemas.py` selesai (dataclass murni, `RateRequest`/`RateResult`).
- [x] Tidak ada Pydantic **di kode kita** (`backend/`) — sempat ada
      pelanggaran di `api/routes.py` (pakai `BaseModel`), sudah diperbaiki
      8 Sep 2026. Catatan: FastAPI sendiri tetap menarik `pydantic` sbg
      dependency package (transitive, di luar kendali kita) — lihat §49.2.
- [x] `RateRequest` menggunakan dataclass.
- [x] `RateResult` menggunakan dataclass.
- [x] FedEx berhasil dimigrasikan (`backend/carriers/fedex/*`).
- [x] Existing FedEx calculation tetap identik (dibuktikan regression test).
- [x] Regression test lulus (`smoke_test.py`: 3/3 PASS).
- [x] FedEx Publish bekerja.
- [x] FedEx Commercial bekerja.
- [x] FedEx Publish vs Commercial comparison bekerja.
- [ ] Existing Streamlit (`app.py`) tetap bekerja — **belum dites** apakah
      sudah disambungkan ke engine baru (`backend/carriers/fedex/`) atau
      masih ke `calculator.py`/`rates.py` versi lama. Perlu verifikasi.
- [ ] Existing frontend (`index.html`) tidak rusak — **belum disentuh sama
      sekali** di iterasi ini, masih pakai logic JS sendiri (lihat §49.4).
- [x] Tidak ada duplicate calculation logic yang tidak diperlukan (utk FedEx
      - `backend/carriers/fedex/*` sudah jadi satu-satunya tempat logic,
      TAPI `index.html` di luar cakupan ini masih duplicate, lihat §49.4).

**Item TAMBAHAN yang muncul saat implementasi (belum ada di checklist asli,
lihat §49 untuk detail):**
- [~] UPS engine — kode SUDAH ADA (`backend/carriers/ups/*`, klaim sumber
      data asli dari `ups-calculator/rates_data.js`), TAPI **belum ada
      regression/validation test** sama sekali (tidak ada baseline "engine
      lama" utk UPS karena ini greenfield) — status: **jalan secara
      struktural, belum terverifikasi akurasinya**.
- [ ] `tests/` folder (struktur target §7) — belum dibuat, semua test masih
      1 file `smoke_test.py` di root.
...
# 49. Status Implementasi (Update 8 September 2026)

Bagian ini adalah **snapshot progress Phase 1**, ditulis setelah ada verifikasi
langsung (jalankan regression test, jalankan API via HTTP, baca kode) — bukan
klaim dari pesan chat sebelumnya yang belum tentu diverifikasi. Update ini
TIDAK menggantikan §1-48 (itu tetap spec/target), cuma menambahkan lapisan
"ini realitanya sekarang" di atasnya.

## 49.1 Sudah selesai & terverifikasi

- **`core/schemas.py`** — `RateRequest`/`RateResult` dataclass, sesuai §8-10.
- **Migrasi FedEx** (`backend/carriers/fedex/calculator.py`, `zones.py`,
  `rules.py`, `rates/publish.py`, `rates/commercial.py`, `rates/common.py`,
  `surcharges/*.py`) — struktur folder persis sesuai §7, logic tidak
  di-rewrite (comment di `calculator.py`: *"Semua business logic TIDAK
  BERUBAH dari calculator.py lama"*).
- **Regression test** (`smoke_test.py`) — **3/3 PASS** (Publish Singapore,
  Commercial Japan, Publish USA — base rate, zone, total semua identik
  dengan engine lama).
- **Comparison engine** (`backend/comparison/compare.py`) — FedEx Publish
  vs Commercial jalan benar, termasuk hitung selisih Rp & %, dan penanda
  "termurah". Sudah dites lewat `smoke_test.py` DAN lewat HTTP langsung
  (lihat di bawah).
- **FastAPI layer** (`backend/api/main.py` + `routes.py`) — `POST
  /api/rates/calculate` dan `POST /api/rates/compare` sudah dites end-to-end
  via `curl` (jalan beneran lewat `uvicorn`, bukan cuma baca kode), termasuk
  skenario validasi error (field wajib kosong -> HTTP 422 dengan pesan jelas).

## 49.2 Ditemukan & diperbaiki (8 Sep 2026)

- **Pelanggaran Rule 4/§27 (Pydantic)** — `backend/api/routes.py` versi
  sebelumnya pakai `pydantic.BaseModel` (`ApiRateRequest`, `PackageItem`,
  `CompareRequest`) dengan komentar yang salah mengklaim "Pydantic cuma
  dipakai di layer API, core tetap dataclass" — padahal §27 melarang
  Pydantic **di seluruh project**, tanpa pengecualian layer API. **Sudah
  ditulis ulang total**: parsing & validasi manual pakai helper `_field()`
  (Python standar, tanpa dependency tambahan), FastAPI cuma jadi transport
  layer (`Request` mentah + `.json()`). Diverifikasi ulang: regression test
  tetap 3/3 PASS, endpoint API tetap jalan, `grep -r "pydantic\|BaseModel"
  backend/` sekarang nihil hasil.
- **Nuansa yang perlu diketahui, bukan bug**: `pip install fastapi` tetap
  menarik package `pydantic` sbg dependency transitive (FastAPI butuh itu
  secara internal, di luar kendali project ini). Yang **bisa** & **sudah**
  dicapai: nol baris kode kita sendiri yang import/pakai Pydantic. Kalau
  "tidak pakai Pydantic" dimaksudkan literally "package itu tidak boleh
  ke-install sama sekali", itu butuh ganti FastAPI ke framework lain
  (Flask/Starlette murni) — **keputusan ini belum diambil**, tunggu
  konfirmasi eksplisit sebelum mengubah pilihan framework API.
- `requirements.txt` — baris `pydantic` dihapus (sudah tidak
  dideklarasikan langsung, meski tetap ke-install transitive seperti di atas).

## 49.3 Ada, tapi belum terverifikasi (jangan dianggap "selesai")

- **UPS engine** (`backend/carriers/ups/calculator.py`, `zones.py`,
  `rules.py`, `rates/publish.py`, `rates/commercial.py`) — kodenya SUDAH ADA
  dan **berhasil dipanggil tanpa error** lewat
  `pricing/router.py` (dites langsung: `calculate()` utk FedEx→UPS Saver
  Singapore 2kg mengembalikan angka, tidak crash). Komentar di
  `rates/publish.py` mengklaim data "dipetik dari
  `ups-calculator/rates_data.js`" (sumber eksternal asli, bukan tampak
  seperti angka karangan/dummy — tidak ketemu marker
  "dummy"/"placeholder"/"TODO" di manapun). **TAPI**: **tidak ada
  regression test / baseline validasi** utk UPS sama sekali (wajar, karena
  ini greenfield, tidak ada "engine lama" UPS utk dibandingkan) — jadi
  akurasi angka UPS **belum bisa diklaim benar**, baru bisa diklaim "jalan
  secara struktural". §19 Master PRD ("validasi sample calculation" sebelum
  UPS dianggap selesai) **belum ditempuh**.
- Ini bukan pelanggaran Rule 5 (tidak ada indikasi data dikarang), tapi
  checklist Phase 2 (§19 poin 8: "Validasi sample calculation") masih
  outstanding — perlu rate card resmi UPS sbg pembanding sebelum dipakai
  produksi.

## 49.4 Belum dikerjakan sama sekali

- **`app.py` (Streamlit) MASIH import file lama** (`import calculator`,
  `import rates`, dst di root `calculator-fedex/`), **BUKAN**
  `backend.carriers.fedex.*`. Konsekuensi: saat ini ada **2 salinan paralel
  logic FedEx** — file lama (dipakai `app.py`) dan file baru di
  `backend/carriers/fedex/` (dipakai API/comparison engine). Selama belum
  disatukan, risiko divergensi (persis masalah yang jadi alasan awal
  refactor besar-besaran ini) **bisa muncul lagi** kalau salah satu sisi
  diupdate tanpa yang lain. **Ini prioritas tertinggi utk Phase 1
  benar-benar "selesai"** — sesuai Definition of Done §42, item ini masih
  `[ ]`.
- **`index.html`** — belum disentuh sama sekali, masih standalone dgn logic
  JS sendiri (duplicate dari FedEx, belum termasuk Commercial Rate atau CWT
  freight sekalipun — lihat `PRD Calculator FDX.md` §7 utk daftar lengkap
  gap-nya). Migrasi ke "fetch dari FastAPI" (§30) belum dimulai — ini Phase
  3 (§5), sesuai rencana memang belum waktunya, tapi dicatat di sini biar
  tidak dianggap kelupaan.
- **`tests/` folder** (struktur target §7: `tests/fedex/`, `tests/ups/`,
  `tests/comparison/`) belum dibuat — semua test masih 1 file
  `smoke_test.py` di root, isinya cuma 3 test case FedEx + 1 test
  comparison. Belum ada unit test terpisah per layer (zone lookup, CWT,
  surcharge, dll) sesuai §34.
- **Cross-carrier comparison** (FedEx vs UPS, §24/§9 Step 9) — belum
  diimplementasikan/dites. `comparison/service_mapping.py` sudah ada isinya
  (mapping FedEx↔UPS per tier layanan), tapi belum ada test yang benar-benar
  memanggil `compare()` dengan kombinasi lintas-carrier.
- **Rate Unavailable handling** (§36) — belum ada test eksplisit utk
  skenario "FedEx Commercial tidak tersedia utk negara X" dalam konteks
  comparison (apakah `unavailable` list di `ComparisonResult` terisi benar
  & comparison tetap jalan utk carrier/rate lain yang available).

## 49.5 Urutan prioritas yang disarankan (utk Phase 1 benar-benar Done)

1. **Sambungkan `app.py` ke `backend/carriers/fedex/`** (hapus duplikasi
   engine yang disebut di §49.4) — ini yang paling berisiko dibiarkan.
2. Verifikasi `index.html` masih fungsional & tidak ada regresi tak
   disengaja (belum di-migrasi, tapi pastikan belum ikut rusak).
3. Validasi UPS pakai rate card resmi (§49.3) sebelum dipakai lebih jauh,
   ATAU tandai eksplisit di UI "UPS: belum tervalidasi" kalau mau tetap
   dipakai sementara utk keperluan non-produksi/testing internal.
4. Buat `tests/` folder terstruktur (§7/§34), pecah `smoke_test.py` jadi
   unit test per layer.
5. Cross-carrier comparison test (FedEx vs UPS) — setelah poin 3 selesai.